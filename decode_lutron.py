...
for msg in msgs.iter('message'):
    ...
    if(ofs > 0):
        ...
        decoded_bytes = b''
        decoded_bytes_msb = b''
        bits = bits[ofs+len(preamble_sync)-11:] #Detect preamble and sync byte, Strip our preamble but keep sync byte
        ...
        calculated_crc = calc_crc(decoded_bytes[3:-2])
        message_crc = (decoded_bytes[-2] << 8) + decoded_bytes[-1]
        print(decoded + ' - CRC match is ' + str(calculated_crc == message_crc))
